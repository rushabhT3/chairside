...
async def test_list_tools_reports_both_servers(youcam: YouCamAdapter, ledger: LocalLedger) -> None:
    ...
    listed = {name.replace("-", "_") for name in tools}
    assert TOOL_SKIN_ANALYSIS in listed
    assert "AI_Clothes_Virtual_Try_On" in listed
...
async def test_color_tones_parses_hex_and_undertone(
    ...
    assert tones.skin_tone == "#b69379"
    assert tones.undertone == "warm"
    assert tones.hair_color_hex == "#5b575a"
...
async def test_skin_hd_has_all_fourteen_concerns_as_ints(youcam: YouCamAdapter) -> None:
    ...
    assert scores.scores["oiliness"] == 1
    assert scores.scores["moisture"] == 81
...
async def test_hair_color_render_carries_tool_server_and_time(youcam: YouCamAdapter) -> None:
    ...
    assert render.image_url.startswith("https://")
...
async def test_other_renders(youcam: YouCamAdapter) -> None:
    assert (await youcam.hairstyle_tryon(SELFIE, "female_s_wave_brunette")).image_url.startswith(
        "https://"
    )
    ...
    assert (await youcam.skin_simulation(SELFIE, "brightening")).image_url.startswith("https://")
