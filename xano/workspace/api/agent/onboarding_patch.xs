query "onboarding" verb=PATCH {
  ...
  stack {
    ...
    db.query "onboarding" {
      where = $db.onboarding.salon_id == $auth.salon_id
      return = { type: "single" }
    } as $existing
    ...
    conditional {
      if ($existing == null) {
        db.add "onboarding" {
          data = { salon_id: $auth.salon_id, steps: $input.steps, state: $input.state, failing_step: $input.failing_step, updated_at: now }
        } as $onboarding
      }
      else {
        db.edit "onboarding" {
